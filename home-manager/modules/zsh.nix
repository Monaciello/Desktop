...
# Aliases assume CLI/GUI tools from `home-manager/packages/` (and Linux-only apps from `apps.nix`).
# Dev-only tools stay in `shells/` — if you drop a package from HM, drop or adjust the matching alias.

let
  isLinux = pkgs.stdenv.hostPlatform.isLinux;
  isDarwin = pkgs.stdenv.hostPlatform.isDarwin;

  flakePath = if isDarwin then "~/Projects/Desktop/.#macbook" else "~/Projects/Desktop/.#alice";
  homeManagerFlake =
    if isDarwin then "~/Projects/Desktop/.#sasha@macbook" else "~/Projects/Desktop/.#sasha@alice";
...
in
{
  programs.zsh = {
    ...
    # Aliases reference tools that must be available in your user PATH (provided by shell.nix or environment).
    shellAliases =
      sharedAliases
